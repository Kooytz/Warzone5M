...
local Lang = {
	...
	position = "Sua posição na fila: %d/%d. Aguarde para conectar.",
	steam = "Abra sua steam e reinicie o FiveM para conectar.",
	connecterror = "Falha ao entrar na fila, apague o seu cachê e tente novamente.",
	error = "Conexão mal sucedida. Verifique se sua steam e social club estão devidamente conectados e tente novamente."
}
...
function isPriority(identifiers)
	...
	for _,v in pairs(identifiers) do
		if string.find(v,"steam") then
			...
			local infoAccount = CORE.infoAccount(splitName[2])
			if infoAccount then
